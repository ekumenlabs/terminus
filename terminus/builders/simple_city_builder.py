...
class SimpleCityBuilder(object):
    ...
    def _create_street_matrix(self, city, size):
        for x in range(1, size):
            road = Street()
            road.add_point(Point(0, x*100))
            road.add_point(Point(100 * size, x*100))
            city.add_road(road)

        for x in range(1, size):
            road = Street()
            road.add_point(Point(x*100, 0))
            road.add_point(Point(x*100, 100 * size))
            city.add_road(road)

    def _create_surrounding_ring_road(self, city, size):
        ring_road_1 = Street('RingRoad1')
        ring_road_1.add_point(Point(-2.5, 0))
        ring_road_1.add_point(Point(size*100+2.5, 0))
        city.add_road(ring_road_1)

        ring_road_2 = Street('RingRoad2')
        ring_road_2.add_point(Point(size*100, 0))
        ring_road_2.add_point(Point(size*100, size*100))
        city.add_road(ring_road_2)

        ring_road_3 = Street('RingRoad3')
        ring_road_3.add_point(Point(-2.5, size*100))
        ring_road_3.add_point(Point(size*100+2.5, size*100))
        city.add_road(ring_road_3)

        ring_road_4 = Street('RingRoad4')
        ring_road_4.add_point(Point(0, 0))
        ring_road_4.add_point(Point(0, size*100))
