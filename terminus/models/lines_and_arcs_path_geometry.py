...
class LinesAndArcsPathGeometry(PathGeometry):
    ...
    def _build_geometry_from(self, points):
        ...
        for index, point in enumerate(points):
            ...
            if self._elements:
                previous_element_point = self._elements[-1].end_point()
            else:
                previous_element_point = previous_point

            if next_point is None:
                self._elements.append(LineSegment(previous_element_point, point))
            else:
                ...
                if abs(angle_between_vectors_in_degrees) < 1e-5:
                    self._elements.append(LineSegment(previous_point, point))
                else:
                    ...
                    if previous_segment.length() < 1e-5:
                        connection_arc = Arc(previous_element_point, self._elements[-1].end_heading(), radius, angle_between_vectors_in_degrees)
                        self._elements.append(connection_arc)
                    else:
                        connection_arc = Arc(previous_segment_new_end_point, previous_segment.end_heading(), radius, angle_between_vectors_in_degrees)
                        self._elements.append(previous_segment)
                        self._elements.append(connection_arc)
    ...
    def _index_nodes(self, nodes):
        ...
        for index, node in enumerate(nodes):
            ...
            if next_node is None:
                mapping[previous_node.center.rounded_to(7)] = previous_node
                mapping[node.center.rounded_to(7)] = node
            else:
                point = node.center
                ...
                if abs(angle_between_vectors_in_degrees) < 1e-5:
                    ...
                    mapping[previous_point.rounded_to(7)] = previous_node
                    mapping[point.rounded_to(7)] = node
                else:
                    inverted_previous_segment = LineSegment(point, previous_point)
                    next_segment = LineSegment(point, next_point)
                    delta = min(inverted_previous_segment.length() / 2.0, next_segment.length() / 2.0, 5)
                    ...
                    if previous_segment.length() < 1e-5:
                        ...
                        mapping[connection_arc.start_point().rounded_to(7)] = node
                        mapping[connection_arc.end_point().rounded_to(7)] = node
                    else:
                        connection_arc = Arc(previous_segment_new_end_point, previous_segment.end_heading(), radius, angle_between_vectors_in_degrees)

                        elements.append(previous_segment)
                        mapping[previous_segment.start_point().rounded_to(7)] = previous_node
