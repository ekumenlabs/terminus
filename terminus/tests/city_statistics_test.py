...
class CityStatisticsTest(unittest.TestCase):
    ...
    def test_empty_city(self):
        ...
        self.assertEquals(values['lane_nodes_count'], 0)
        self.assertEquals(values['lane_intersections_count'], 0)
        self.assertEquals(values['average_lane_intersections'], 0)
        self.assertEquals(values['average_lane_nodes'], 0)

    def test_sample_city(self):
        city = City()

        city.add_road(Street.from_points([Point(-100, 0), Point(0, 0), Point(100, 0)]))
        city.add_road(Trunk.from_points([Point(0, 100), Point(0, 0), Point(0, -100)]))
        city.add_intersection_at(Point(0, 0))

        city.add_road(Street.from_points([Point(-10, 200), Point(0, 200), Point(10, 220), Point(20, 270)]))

        stats = CityStatistics(city)
        values = stats.run()
        self.assertEquals(values['roads_count'], 3)
        self.assertEquals(values['lanes_count'], 4)
        self.assertEquals(values['buildings_count'], 0)
        self.assertEquals(values['blocks_count'], 0)
        self.assertEquals(values['lane_nodes_count'], 14)
        self.assertEquals(values['lane_intersections_count'], 4)
        self.assertEquals(values['average_lane_intersections'], 1.0)
        self.assertEquals(values['average_lane_nodes'], 14.0 / 4.0)
